...
*   **Pre-Processing Pipeline:** Vor der OCR-Anwendung müssen folgende Schritte durchlaufen werden:
    1.  **Grayscale Conversion:** Reduktion auf Graustufen.
    2.  **Inversion:** Umkehrung der Farben (Weiß auf Schwarz -> Schwarz auf Weiß), da Tesseract dies bevorzugt.
    3.  **Rescaling:** Vergrößerung des Bildes (Upsampling) um Faktor 2-3x, da CLI-Schriften bei 1080p oft nur wenige Pixel breit sind und Antialiasing die Kanten für OCR undeutlich macht.
    4.  **Thresholding:** Binarisierung (Otsu's Binarization) zur Entfernung von Hintergrundrauschen (z.B. Transparenzeffekte in modernen Terminals).
...
1.  **Observe:** `capture_screen` aufrufen.
2.  **Orient:** OCR-Ergebnis analysieren. Prüfen auf Schlüsselwörter (Prompts wie `C:\>`, Fehlercodes, "Press any key").
3.  **Decide:** LLM vergleicht Ist-Zustand mit Ziel.
    *   *Beispiel:* Ziel ist "Ordner löschen". Ist-Zustand zeigt "Access Denied". Entscheidung: "Versuche `takeown` Befehl".
4.  **Act:** `inject_keystrokes` ausführen.
5.  **Verify:** Erneuter `capture_screen` nach Verzögerung, um zu prüfen, ob die Eingabe erschienen ist (Echo-Check).
